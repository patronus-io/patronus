class Reviewership < ActiveRecord::Base
  ...
  def ensure_webhook_installed!
    hook_url = Rails.application.routes.url_helpers.url_for controller: "github_web_hook", action: "create", host: ENV['GITHUB_WEBHOOK_HOST'.freeze]
  ...
  def ensure_bot_permissions!
    unless user.github.collaborator?(repo.name, ENV['GITHUB_BOT_USERNAME'.freeze])
      # adds bot to repo with push access, even on organization-owned repos
      user.github.add_collaborator(repo.name, ENV['GITHUB_BOT_USERNAME'.freeze])
    end
  end
end
